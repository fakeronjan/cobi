...
od_goal_cap      = None # optional per-side goal clip for the O/D lean only. MLS is
                          # a single balanced league with no minnow routs, so raw
                          # goals are fine; left as a knob in case a 6-0 distorts.
...
def run_pipeline(scrape=True):
    ...
    for i in range(1, max_date_id + 1):
        ...
        try:
            ...
            # rating_o + rating_d == the solved rating. delta is per team:
            # adding it to both sides keeps the attack-vs-defense lean
            # (rating_o - rating_d) while pinning the level to the rating, so
            # the cap / shootout treatment flows through. sum(delta) = 0, so
            # both vectors stay zero-sum.
            ranked_od = _solve_wls_od(working, off_share=od_off_share, goal_cap=od_goal_cap)
    ...
    # Normalize the 'name' merge_asof key too: on a full rebuild cobi_df is
    # seeded from an empty pd.DataFrame(columns=[...]) (object dtype), so concat
    # downcasts 'name' to object, while lastmatch's 'name' is str (read_csv under
    # pandas-3 infer_string). merge_asof requires matching 'by' keys.
    ...
    lastmatch_sorted['name']   = lastmatch_sorted['name'].astype(str)
